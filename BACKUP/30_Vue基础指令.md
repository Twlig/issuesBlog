...
- 没有key属性
  ...
  按照节点相同且都是text类型的规则，会直接按照新虚拟节点的文本内容替换对应的文本。渲染输出的更新步骤如下：

  原先内容为1的`<div>`元素内容变成0，原先内容为2的`<div>`元素内容变成1，原先内容为3的`<div>`元素内容变成2，最后新增一个`<div>`元素，内容为3。

- 有key属性

  - 当为v-for添加了key属性后，新旧节点对比时，虽然都是div，但**key不同，会被当做不同节点处理**。
  - 新虚拟节点中的key为0的第一个节点没有在旧虚拟节点列表中找到对应。
  - 会新增一个key为0的节点，插入到未处理的节点之前，即1之前。
